...
function Customerupdateform() {
    ...
    useEffect(() => {
      const fetchData = async () => {
        try {
          ...
          setCustomer_code(response.data.customer_code)
          setService(response.data.service)
          setPackages(response.data.package)
          setRegistered_date(response.data.registered_date)
          setCustomer_name(response.data.customer_name)
          setUsername(response.data.username)
          setAddress_1(response.data.address_1)
          setName(response.data.name)
          setPrimary_mobile(response.data.primary_mobile)
          setSecondary_con(response.data.secondary_con)
          setChange_user(response.data.change_user)
          setCustomer_notes(response.data.customer_notes)
          setBranch(response.data.branch)
          setCustomer_type(response.data.customer_type)
          setPackage_period(response.data.package_period)
          setCompany(response.data.company)
          setLast_name(response.data.last_name)
          setPassword(response.data.password)
          setAddress_2(response.data.address_2)
          setLandline(response.data.landline)
          setSecondary_mob(response.data.secondary_mob)
          setEnable_portal_log(response.data.enable_portal_log)
          setSend_sms(response.data.send_sms)
        } catch (error) {
    ...
    const handleSubmit = (e) => {
      ...
      if (validate()) {
        axios.put('http://116.90.227.238:3000/Customers', {
